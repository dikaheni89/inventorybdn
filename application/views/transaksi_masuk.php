    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="alert alert-primary" role="alert">
                <strong>Info :</strong> Halaman ini digunakan untuk menambahkan Transaksi Inventory masuk!
            </div>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <h5 class="card-title mb-0">Transaksi Masuk</h5>
                            ...
                            <table id="table-user" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-ordering="false" style="width: 1%">&nbsp;</th>
                                    </tr>
                                </thead>
                            </table>
                        </div>
                        ...
                        <div class="col-xxl-12">
                            <div>
                                ...
                                <select name="sparepart" id="sparepart" class="form-control">
                                    ...
                                    ?>
                                        <option value="<?= $key->kode_barang . '*' . $key->nama_barang . '*' . $key->kategori  ?>"><?= $key->kode_barang . ' - ' . $key->nama_barang ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
                        ...
                        <div class="col-xxl-12">
                            <div>
                                ...
                                <select name="suplier" id="suplier" class="form-control">
                                    ...
                                    ?>
                                        <option value="<?= $key->suplier ?>"><?= $key->kode_suplier . ' - ' . $key->suplier ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
...
<script>
    ...
    $(function() {
        $("#table-user").DataTable({
            ...
            'ajax': {
                'dataType': 'json',
                'url': '<?= base_url() ?>dashboard/ajax_table_transaksi_masuk',
                'type': 'post',
            },
            'columns': [{
                "target": [<?= $target ?>],
                ...
                "data": "data",
                "render": function(data) {
                    var html = `<button type="button" class="btn btn-sm btn-danger" onclick="promp_delete('${data.id}', '${data.lokasi}', '${data.kode_barang}', '${data.jumlah}')">Hapus</button>`
                    return html
                }
            }, ],
    ...
    $("#form-data").submit(function(e) {
        ...
        var url_ajax = '<?= base_url() ?>inventory/insert_transaksi_masuk'
        // var url_ajax = '<?= base_url() ?>dashboard/insert_data_masuk'
    ...
    function promp_delete(id, lokasi, kode_barang, qty) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: base_url + 'inventory/delete_transaksi_masuk',
                    data: {
                        id: id,
                        lokasi: lokasi,
