    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="alert alert-primary" role="alert">
                <strong>Info :</strong> Halaman ini digunakan untuk menambahkan dan menghapus satuan!
            </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            ...
                            <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#exampleModalgrid">
                                <i class="las la-plus-circle"></i> Tambah Satuan
                            </button>
                        </div>
                            ...
                            <table id="table-user" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-ordering="false">Kode Satuan</th>
                ...
                <form id="form-data">
                    <div class="row g-3">
                        <div class="col-xxl-12">
                            <div>
                                <label for="kode_satuan" class="form-label">Nama Satuan</label>
                                <input type="text" class="form-control" id="kode_satuan" onkeyup="this.value = this.value.toUpperCase()">
                            </div>
                        </div><!--end col-->
                        <div class="col-xxl-12">
                            <div>
                                <label for="deskripsi" class="form-label">Deskripsi</label>
                                <input type="text" class="form-control" id="deskripsi" onkeyup="this.value = this.value.toUpperCase()">
                            </div>
                        </div><!--end col-->
                        <div class="col-lg-12">
...
<script>
    ...
    $(function() {
        $("#table-user").DataTable({
            ...
            'ajax': {
                'dataType': 'json',
                'url': '<?= base_url() ?>dashboard/ajax_table_satuan',
                'type': 'post',
            },
            'columns': [{
                "target": [<?= $target ?>],
                ...
                "render": function(data) {
                    if (user == 'administrator') {
                        return `<button type="button" class="btn btn-sm btn-danger" onclick="delete_data('` + data.id + `','` + data.kode_satuan + `','` + data.deskripsi + `')"><i class="ri-delete-bin-line"></i> Hapus</button>`
                    } else {
                        return `-`
                    }
                }
    ...
    $("#form-data").submit(function(e) {
        ...
        if ($('#kode_satuan').val() == '' || $('#deskripsi').val() == '') {
            Swal.fire(
                'error!',
    ...
    function delete_data(id, kode, deskripsi) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url() ?>dashboard/delete_data_satuan',
                    data: {
                        id: id,
                        kode: kode,
                        deskripsi: deskripsi,
