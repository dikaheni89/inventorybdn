    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="alert alert-primary" role="alert">
                <strong>Info :</strong> Halaman ini digunakan untuk menambahkan dan menghapus agen!
            </div>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <h5 class="card-title mb-0">Agen</h5>
                            ...
                            <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#exampleModalgrid">
                                <i class="las la-plus-circle"></i> Tambah Agen
                            </button>
                        </div>
                            ...
                            <table id="table-user" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-ordering="false">No. Identitas</th>
                                        <th data-ordering="false">Alamat</th>
                                        <th data-ordering="false">Handphone</th>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalgridLabel">Tambah Agen</h5>
...
<script>
    ...
    $(function() {
        $("#table-user").DataTable({
            ...
            'ajax': {
                'dataType': 'json',
                'url': '<?= base_url() ?>dashboard/ajax_table_agen',
                'type': 'post',
            },
            'columns': [{
                "target": [<?= $target ?>],
                ...
                "render": function(data) {
                    if (data.layanan == 'HOTSPOT') {
                        return `<span class="badge text-bg-success">HOTSPOT</span>`
                    } else {
                        return `<span class="badge text-bg-secondary">PPPOE</span>`
                    }
                }
            }, {
                "target": [<?= $target ?>],
                ...
                "data": "data",
                "render": function(data) {
                    return `<button type="button" class="btn btn-sm btn-danger" onclick="delete_data('` + data.kode + `','` + data.nama + `')"><i class="ri-delete-bin-line"></i> Hapus</button>`
                }
            }, ],
    ...
    $("#form-data").submit(function(e) {
        ...
        if ($('#nama').val() == '' || $('#no_identitas').val() == '' || $('#alamat').val() == '' || $('#handphone').val() == '') {
            Swal.fire(
                'error!',
        ...
        form_data.append('table', 'agen');
        form_data.append('nama', $("#nama").val());
        form_data.append('no_identitas', $("#no_identitas").val());
        form_data.append('alamat', $("#alamat").val());
        form_data.append('handphone', $("#handphone").val());
        form_data.append('layanan', $("#layanan").val());

        var url_ajax = '<?= base_url() ?>dashboard/insert_data_agen'
    ...
    function delete_data(kode, nama) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url() ?>dashboard/delete_data_agen',
                    data: {
                        kode: kode,
                        nama: nama,
                        table: "agen"
