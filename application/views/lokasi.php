    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="alert alert-primary" role="alert">
                <strong>Info :</strong> Halaman ini digunakan untuk menambahkan dan menghapus Lokasi gudang atau rak!
            </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            ...
                            <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#exampleModalgrid">
                                <i class="las la-plus-circle"></i> Tambah Lokasi
                            </button>
                        </div>
                            ...
                            <table id="table-user" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-ordering="false">Nama Lokasi</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                            </table>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalgridLabel">Tambah Lokasi</h5>
                    ...
                    <div class="row g-3">
                        <div class="col-xxl-12">
                            <div>
                                ...
                                <input type="text" class="form-control" id="lokasi" onkeyup="this.value = this.value.toUpperCase()">
                            </div>
                        </div><!--end col-->
                        <div class="col-lg-12">
...
<script>
    ...
    $(function() {
        $("#table-user").DataTable({
            ...
            'ajax': {
                'dataType': 'json',
                'url': '<?= base_url() ?>dashboard/ajax_table_lokasi',
                'type': 'post',
            },
            'columns': [{
                "target": [<?= $target ?>],
                ...
                "render": function(data) {
                    if (user == 'administrator') {
                        return `<button type="button" class="btn btn-sm btn-danger" onclick="delete_data('` + data.id + `','` + data.lokasi + `')"><i class="ri-delete-bin-line"></i> Hapus</button>`
                    } else {
                        return `-`
                    }
                }
    ...
    $("#form-data").submit(function(e) {
        ...
        form_data.append('table', 'lokasi');
        form_data.append('lokasi', $("#lokasi").val());

        var url_ajax = '<?= base_url() ?>dashboard/insert_data_lokasi'
    ...
    function delete_data(id, lokasi) {
        // alert(lokasi)
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url() ?>dashboard/delete_data_lokasi',
                    data: {
                        id: id,
                        lokasi: lokasi,
                        table: "lokasi"
