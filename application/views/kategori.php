    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="alert alert-primary" role="alert">
                <strong>Info :</strong> Halaman ini digunakan untuk menambahkan dan menghapus kategori!
            </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            ...
                            <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#exampleModalgrid">
                                <i class="las la-plus-circle"></i> Tambah Kategori
                            </button>
                        </div>
                    ...
                    <div class="row g-3">
                        <div class="col-xxl-12">
                            <div>
                                ...
                                <input type="text" class="form-control" id="kategori" name="kategori" onkeyup="this.value = this.value.toUpperCase()">
                            </div>
                        </div><!--end col-->
                        <div class="col-lg-12">
...
<script>
    ...
    $(function() {
        $("#table-user").DataTable({
            ...
            'ajax': {
                'dataType': 'json',
                'url': '<?= base_url() ?>dashboard/ajax_table_kategori',
                'type': 'post',
            },
            'columns': [{
                "target": [<?= $target ?>],
                ...
                "render": function(data) {
                    if (user == 'administrator') {
                        return `<button type="button" class="btn btn-sm btn-danger" onclick="delete_data('` + data.id + `','` + data.kategori + `')"><i class="ri-delete-bin-line"></i> Hapus</button>`
                    } else {
                        return `-`
                    }
                }
    ...
    $("#form-data").submit(function(e) {
        ...
        form_data.append('table', 'kategori');
        form_data.append('kategori', $("#kategori").val());

        var url_ajax = '<?= base_url() ?>dashboard/insert_data_kategori'
    ...
    function delete_data(id, kategori) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url() ?>dashboard/delete_data_kategori',
                    data: {
                        id: id,
                        kategori: kategori,
                        table: "kategori"
