                            ...
                            <table id="table-user" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-ordering="false" style="width: 20%;">Nama</th>
            ...
            <div class="modal-body">
                <form id="form-data-edit">
                    <div class="row g-3">
                        ...
                        <div class="col-xxl-12">
                            <div>
                                <label for="file_edit" class="form-label">Photo</label>
                                <input class="form-control" type="file" id="file_edit" name="file_edit">
                            </div>
                        </div><!--end col-->
                        <div class="col-xxl-12">
                            <div>
                                <label for="userid_edit" class="form-label">User ID</label>
                                <input type="text" class="form-control" id="userid_edit" readonly>
                            </div>
                        </div><!--end col-->
                        <div class="col-xxl-12">
                            <label for="role_id_edit" class="form-label">Hak Akses</label>
...
<script>
    ...
    $(function() {
        $("#table-user").DataTable({
            ...
            'columns': [{
                ...
                "render": function(data) {
                    return `<div class="d-flex gap-2 align-items-center">
                                <div class="flex-shrink-0">
                                    <img src="<?= base_url('assets/default/assets/images/users/') ?>` + data.photo + `" alt="" class="avatar-xs rounded-circle">
                                </div>
                ...
                "data": "data",
                "render": function(data) {
                    return `<button type="button" class="btn btn-sm btn-success" onclick="edit_data('` + data.id + `','` + data.userid + `')"><i class="ri-delete-bin-line"></i> Edit</button>&nbsp;<button type="button" class="btn btn-sm btn-danger" onclick="delete_data('` + data.id + `','` + data.userid + `')"><i class="ri-delete-bin-line"></i> Hapus</button>`
                }
            }, ],
    ...
    function delete_data(id, userid) {
        ...
        $.ajax({
            ...
            success: function(result) {
                result.forEach(d => {
                    ...
                    $('#userid_edit').val(d.userid)
                    $('#role_id_edit').val(d.role_id)
                });
            }
        })
    }
    ...
    $("#form-data-edit").submit(function(e) {
        ...
        form_data.append('userid', $("#userid_edit").val());
        form_data.append('password', $("#password_edit").val());
        form_data.append('role_id', $("#role_id_edit").val());
        if ($('#file_edit').val() !== "") {
            var file_data = $('#file_edit').prop('files')[0];
            form_data.append('file', file_data);
        }

        var url_ajax = '<?= base_url() ?>dashboard/update_data_user'
